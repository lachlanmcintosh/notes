...
\begin{compactitem}
Mercury views \dw{I/O predicates} as describing a relationship between two states of the world: the one before the action and the one after. A state of the world is represented by a value of the special type \e{io}.
...
\time \dw{compilation}: \e{mmc prog.m}
...
\itme \dw{fat lists} are lists that can contian more than one item. Functinos operating on fat lists need to consider extra cases, standard library functions thus will not be appropiate. 
